// ============================================
// GJob.in - Admit Card Single Post SSR (अपडेटेड)
// ============================================
...
export async function onRequest(context) {
  ...
  const canonical = `${env.SITE_URL}/admit-card/${slug}/`;

  // Breadcrumb
  const breadcrumbItems = [
    { name: 'Home', url: env.SITE_URL },
    { name: 'Admit Card', url: `${env.SITE_URL}/category/admit-card/` },
  ...
  const meta = generateMetaTags(env, {
    ...
    type: 'article',
    keywords: post.meta_keywords || 'admit card, hall ticket, ' + post.title,
...
<body>
  ...
  <div class="container">
    <article class="post">
      ...
      <div class="meta">📅 ${new Date(post.created_at).toLocaleDateString('en-IN')} | 📂 Admit Card | 👁️ ${post.views} views</div>
      ${countdownHTML}
      ...
      ${post.apply_link ? `<a href="${sanitizeHTML(post.apply_link)}" class="btn-apply" target="_blank" rel="nofollow noopener">🎫 Download Admit Card</a>` : ''}
