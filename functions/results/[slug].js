// ============================================
// GJob.in - Results Single Post SSR (अपडेटेड)
// ============================================
...
export async function onRequest(context) {
  ...
  const canonical = `${env.SITE_URL}/results/${slug}/`;

  // Breadcrumb
  const breadcrumbItems = [
    { name: 'Home', url: env.SITE_URL },
    { name: 'Results', url: `${env.SITE_URL}/category/results/` },
  ...
  const meta = generateMetaTags(env, {
    ...
    type: 'article',
    keywords: post.meta_keywords || 'result, score card, ' + post.title,
...
<body>
  ...
  <div class="container">
    <article class="post">
      ...
      <div class="meta">📅 ${new Date(post.created_at).toLocaleDateString('en-IN')} | 📂 Results | 👁️ ${post.views} views</div>
      ${countdownHTML}
      ...
      ${post.apply_link ? `<a href="${sanitizeHTML(post.apply_link)}" class="btn-apply" target="_blank" rel="nofollow noopener">📥 Download Result</a>` : ''}
